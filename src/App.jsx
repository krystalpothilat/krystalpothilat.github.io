...
export default function App() {
  const {
    ...
    unlockAll, resetPuzzle, completedSections, totalCompletion,
    justSnapped,
  } = usePuzzleEngine()
  ...
  const handlePieceClick = useCallback((piece) => {
    ...
    if (piece.linksTo === '__easter_egg__') {
      unlockAll(); viewport.zoomOut(); return
    }
    if (SECTIONS[piece.linksTo]) {
      viewport.navigateTo(piece.linksTo); return
    }
    setActiveDetail(piece.linksTo)
  }, [viewport, unlockAll, justSnapped])

  return (
    <div className="app-container" style={{ backgroundImage: `url(${IMAGE_URL})` }}>
      <PuzzleBoard
        pieces={pieces} dragging={dragging}
        startDrag={startDrag} moveDrag={moveDrag} endDrag={endDrag}
        viewport={viewport} onPieceClick={handlePieceClick}
      ...
      />
      {achievement && (
        <SectionComplete sectionLabel={achievement.label} onDone={() => setAchievement(null)} />
      )}
    </div>
  )
}