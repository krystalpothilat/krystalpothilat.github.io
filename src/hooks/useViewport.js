...
import { SECTIONS, PIECE_SIZE, SECTION_COLS, SECTION_ROWS, PUZZLE_COLS, PUZZLE_ROWS } from '../data/puzzleData.js'
...
function sectionTranslate(sectionId, scale) {
  ...
  const sectionWidth  = SECTION_COLS * PIECE_SIZE
  const sectionHeight = SECTION_ROWS * PIECE_SIZE

  // Puzzle position of section's center
  const centerX = section.puzzleCol * PIECE_SIZE + sectionWidth / 2
  const centerY = section.puzzleRow * PIECE_SIZE + sectionHeight / 2

  // Translate so section center is in middle of screen
  return {
    x: CW() / 2 / scale - centerX,
    y: CH() / 2 / scale - centerY,
  }
}

// Translate so the whole puzzle is centered
function puzzleTranslate(scale) {
  return {
    x: CW() / 2 / scale - (PUZZLE_COLS * PIECE_SIZE) / 2,
    y: CH() / 2 / scale - (PUZZLE_ROWS * PIECE_SIZE) / 2,
  }
}
...
export function useViewport() {
  ...
  const [translate, setTranslate]   = useState(() => sectionTranslate('home', sectionScale()))
  ...
  const zoomInToSection = useCallback((sectionId) => {
    navigateTo(sectionId)
  }, [navigateTo])

  // Screen → puzzle coordinate conversion (for drag)
  const screenToPuzzle = useCallback((screenX, screenY) => ({
    x: screenX / scale - translate.x,
    y: screenY / scale - translate.y,
  }), [scale, translate])

  return {
    currentSection, viewState, scale, translate,
    navigateTo, zoomOut, zoomInToSection, screenToPuzzle,
