import {
  SECTIONS, PIECE_SIZE, SECTION_COLS, SECTION_ROWS, PUZZLE_COLS, PUZZLE_ROWS,
  SECTION_BOUNDARY_COLS, SECTION_BOUNDARY_ROWS,
...
function flatRight(col)  { return col === PUZZLE_COLS - 1 }
function flatLeft(col)   { return col === 0 }
function flatBottom(row) { return row === PUZZLE_ROWS - 1 }
function flatTop(row)    { return row === 0 }
...
export function getPieceClipPath(col, row, size = PIECE_SIZE) {
  ...
  const H = s * 0.20  // tab curve control
...
function nub(side, s, T, N, H, outward) {
  ...
  switch (side) {
    case 'top': {
      const ty = -T * d
      return [`L ${m-N},0`, `C ${m-N},${ty*0.45} ${m-H},${ty*0.95} ${m},${ty}`, `C ${m+H},${ty*0.95} ${m+N},${ty*0.45} ${m+N},0`, `L ${s},0`].join(' ')
    }
    case 'right': {
      const tx = T * d
      return [`L ${s},${m-N}`, `C ${s+tx*0.45},${m-N} ${s+tx*0.95},${m-H} ${s+tx},${m}`, `C ${s+tx*0.95},${m+H} ${s+tx*0.45},${m+N} ${s},${m+N}`, `L ${s},${s}`].join(' ')
    }
    case 'bottom': {
      const ty = T * d
      return [`L ${m+N},${s}`, `C ${m+N},${s+ty*0.45} ${m+H},${s+ty*0.95} ${m},${s+ty}`, `C ${m-H},${s+ty*0.95} ${m-N},${s+ty*0.45} ${m-N},${s}`, `L 0,${s}`].join(' ')
    }
    case 'left': {
      const tx = -T * d
      return [`L 0,${m+N}`, `C ${tx*0.45},${m+N} ${tx*0.95},${m+H} ${tx},${m}`, `C ${tx*0.95},${m-H} ${tx*0.45},${m-N} 0,${m-N}`, `L 0,0`].join(' ')
    }
    default: return ''
  }
}
...
export function buildPuzzlePieces() {
    ...
    for (let pc = 0; pc < PUZZLE_COLS; pc++) {
        ...
        puzzlePieces[pr][pc] = { top, right, bottom, left, puzzleRow: pr, puzzleCol: pc }
    }
    }
  const pieces = []
  ...
  Object.values(SECTIONS).forEach(section => {
    ...
    const overrideMap = {}
    ;(section.pieces || []).forEach(p => { overrideMap[`${p.localCol},${p.localRow}`] = p })
    ...
    for (let r = 0; r < SECTION_ROWS; r++) {
      for (let c = 0; c < SECTION_COLS; c++) {
        ...
        // Determine edges
        const topEdge = (r === 0)
        ? (edgeMap[`${pc},${pr-1}`]?.bottom !== undefined ? !edgeMap[`${pc},${pr-1}`].bottom : null)
        : !edgeMap[`${pc},${pr-1}`].bottom

        const leftEdge = (c === 0)
        ? (edgeMap[`${pc-1},${pr}`]?.right !== undefined ? !edgeMap[`${pc-1},${pr}`].right : null)
        : !edgeMap[`${pc-1},${pr}`].right

        const rightEdge = (c === SECTION_COLS-1 || flatRight(pc)) 
        ? null 
        : Math.random() < 0.5

        const bottomEdge = (r === SECTION_ROWS-1 || flatBottom(pr)) 
        ? null 
        : Math.random() < 0.5

        edgeMap[`${pc},${pr}`] = { top: topEdge, right: rightEdge, bottom: bottomEdge, left: leftEdge }

        const homeX = pc * PIECE_SIZE
        const homeY = pr * PIECE_SIZE
        const isLocked = pureFun ? true : (override.locked !== undefined ? override.locked : true)
        ...
        pieces.push({
          ...
          homeX, homeY,
          x: positionOffsetX, y: positionOffsetY,
          locked: isLocked,
          connected: isLocked,
          bgX: -homeX, bgY: -homeY,
          ...
          clipPath: getPieceClipPathFromEdges(PIECE_SIZE, edgeMap[`${pc},${pr}`]),
        })
      }
    }
  })
...
export const SNAP_THRESHOLD = PIECE_SIZE * 0.4
...
function getPieceClipPathFromEdges(s, edges) {
  ...
  const top    = edges.top    === null ? `L ${s},0`    : nub('top', s, T, N, H, edges.top)
  const right  = edges.right  === null ? `L ${s},${s}` : nub('right', s, T, N, H, edges.right)
  const bottom = edges.bottom === null ? `L 0,${s}`    : nub('bottom', s, T, N, H, edges.bottom)
