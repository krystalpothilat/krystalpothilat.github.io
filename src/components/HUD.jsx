  ...
  return (
    <>
      <div className={styles.topBar}>
        ...
        <div className={styles.controls}>
          <div
            ...
            style={{
              color: pct === 100 ? 'rgba(80,200,120,0.9)' : 'rgba(255,255,255,0.4)',
          ...
          <div className={styles.menuContainer}>
            <button onClick={() => setShowMenu(v => !v)} className={styles.menuButton}>⋯</button>
            {showMenu && (
              <div className={styles.menuDropdown}>
                <MenuSection label="Navigate">
                  {Object.values(SECTIONS).filter(s => !s.pureFun).map(s => (
                    <MenuItem
                      key={s.id}
                      label={s.label}
                      done={completedSections.has(s.id)}
                      onClick={() => { onNavigate(s.id); setShowMenu(false) }}
                    />
                  ))}
                </MenuSection>
                <div style={{ height: 1, background: 'rgba(255,255,255,0.08)', margin: '4px 0' }} />
                <MenuSection label="Puzzle">
                  <MenuItem label="🔓 Unlock all pieces" onClick={() => { onUnlockAll(); setShowMenu(false) }} />
                  <MenuItem label="↺ Reset puzzle" onClick={() => { onReset(); setShowMenu(false) }} danger />
                </MenuSection>
              </div>
            )}
          </div>
      ...
      <div className={styles.tooltip}>
        {isZoomedOut ? 'click a section to zoom in' : 'drag ● pieces to solve · click tiles to explore'}
      </div>
    </>
  )
}
