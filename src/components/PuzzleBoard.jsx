...
import { PIECE_SIZE, PUZZLE_COLS, PUZZLE_ROWS, SECTION_COLS, SECTION_ROWS, SECTIONS } from '../data/puzzleData.js'

const IMAGE_URL = import.meta.env.VITE_PUZZLE_IMAGE_URL || '/puzzle-image.jpg'

export default function PuzzleBoard({
  pieces, dragging, startDrag, moveDrag, endDrag,
  viewport, onPieceClick, completedSections,
}) {
  const boardRef = useRef(null)
  const screenToPuzzle = useCallback((x, y) => viewport.screenToPuzzle(x, y), [viewport])

  const onMD = useCallback((id, x, y) => startDrag(id, x, y, screenToPuzzle), [startDrag, screenToPuzzle])
  const onMM = useCallback((e) => { if (dragging) moveDrag(e.clientX, e.clientY, screenToPuzzle) }, [dragging, moveDrag, screenToPuzzle])
  const onMU = useCallback(() => endDrag(), [endDrag])
  const onTS = useCallback((id, x, y) => startDrag(id, x, y, screenToPuzzle), [startDrag, screenToPuzzle])
  ...
  return (
    <div ref={boardRef} style={{ position: 'fixed', inset: 0, overflow: 'hidden', background: '#0a0a0f', cursor: dragging ? 'grabbing' : 'default' }}>
      ...
      <div style={{
        ...
        <motion.div
          animate={{
            ...
            x: viewport.translate.x * viewport.scale,
            y: viewport.translate.y * viewport.scale,
          }}
          transition={{ type: 'spring', stiffness: 200, damping: 30, mass: 0.9 }}
          style={{ position: 'absolute', transformOrigin: '0 0', width: puzzleW, height: puzzleH }}
        >
          {pieces.map(piece => (
            <PuzzlePiece
              key={piece.id}
          ...
          return (
            ...
            >
              <span style={{
                ...
                color: done ? 'rgba(80,200,120,0.9)' : 'rgba(255,255,255,0.5)',
